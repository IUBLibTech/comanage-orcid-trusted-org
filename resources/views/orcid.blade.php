...
@if($status === 'success')
    ...
    <h3>Employment</h3>
    @if(!empty($data['employments']['employment']))
        <ul>
            @foreach ($data['employments']['employment'] as $employment)
                <li>
                    <strong>Organization:</strong> {{ $employment['organization-name'] ?? 'N/A' }}<br>
                    <strong>Put Code:</strong> {{ $employment['put-code'] ?? 'N/A' }}<br>
                    <strong>Type:</strong> {{ $employment['type'] ?? 'N/A' }}<br>
                    <strong>Validated:</strong> {{ $employment['validated'] ? 'Yes' : 'No' }}
                </li>
            @endforeach
        </ul>
    @else
    ...
    <p>{{ $data['professional-activities']['count'] > 0 ? 'Present' : 'None found' }}</p>

    <h3>Fundings</h3>
    <ul>
        <li>Self-Asserted: {{ $data['fundings']['self-asserted-count'] ?? 0 }}</li>
        <li>Validated: {{ $data['fundings']['validated-count'] ?? 0 }}</li>
    </ul>

    <h3>Works</h3>
    <ul>
        <li>Self-Asserted: {{ $data['works']['self-asserted-count'] ?? 0 }}</li>
        <li>Validated: {{ $data['works']['validated-count'] ?? 0 }}</li>
    </ul>

    <h3>Peer Reviews</h3>
    <ul>
        <li>Publication Grants: {{ $data['peer-reviews']['peer-review-publication-grants'] ?? 0 }}</li>
        <li>Self-Asserted: {{ $data['peer-reviews']['self-asserted-count'] ?? 0 }}</li>
        <li>Total: {{ $data['peer-reviews']['total'] ?? 0 }}</li>
    </ul>

    <h3>Email Domains</h3>
    <p>{{ ($data['email-domains']['count'] ?? 0) > 0 ? 'Available' : 'None found' }}</p>

    <h3>Education & Qualifications</h3>
    <p>{{ ($data['education-qualifications']['count'] ?? 0) > 0 ? 'Available' : 'None found' }}</p>

    <h3>Research Resources</h3>
    <ul>
        <li>Self-Asserted: {{ $data['research-resources']['self-asserted-count'] ?? 0 }}</li>
        <li>Validated: {{ $data['research-resources']['validated-count'] ?? 0 }}</li>
    </ul>
</div>
@endif
